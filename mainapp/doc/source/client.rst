...
``python client.py -ip -port -n или --name {имя пользователя} -p или -password {пароль}``

1. -ip - адрес сервера сообщений.
2. -port - порт по которому принимаются подключения
